...
// ✅ FIX: Ensure Web Crypto API exists for Baileys
if (!globalThis.crypto?.subtle) {
  try {
    const { webcrypto } = require("crypto");
    globalThis.crypto = webcrypto;
    console.log("[🔧] Patched globalThis.crypto for Baileys compatibility.");
  } catch (e) {
    console.error("[❌] Failed to patch crypto:", e.message);
  }
}
...
async function loadSession() {
  try {
    ...
    if (config.SESSION_ID.startsWith("Akida~")) {
      ...
      const megaFileId = config.SESSION_ID.replace("Akida~", "");
      ...
      throw new Error("Invalid SESSION_ID format. Use 'Akida~' for base64 or 'Mercedes~' for MEGA.nz");
    }
  } catch (error) {
